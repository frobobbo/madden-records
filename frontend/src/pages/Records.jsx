  ...
  return (
    <div className="rounded-xl overflow-hidden shadow-sm border border-gray-200">
      ...
      {sorted.map(([player, value], i) => (
        <div key={player} className={`flex items-center gap-3 px-4 py-3 bg-white ${i > 0 ? 'border-t border-gray-100' : ''}`}>
          <TrophyIcon gold={i === 0} />
          <span className="font-medium text-gray-800 flex-1">{player}</span>
          <span className="text-xl font-bold text-gray-900">{format(value)}</span>
        </div>
      ))}
      {!sorted.length && <div className="px-4 py-3 bg-white text-gray-400 text-sm">No data yet</div>}
    </div>
  );
}
...
function TrophyIcon({ gold }) {
  return (
    <svg width="28" height="28" viewBox="0 0 24 24" fill={gold ? '#f59e0b' : '#9ca3af'}>
